...
protocol_magic = 'MOG'
protocol_version = 1
protocol_header_size = 10
protocol_max_payload_size = 1024 * 1024
protocol_type_status = 1
protocol_type_command = 2
protocol_type_hello = 3
ashita_command_mode_typed = (type(CommandMode) == 'table' and CommandMode.Typed) or 1
ashita_command_mode_macro = (type(CommandMode) == 'table' and CommandMode.Macro) or 2
protocol_value_null = 0
protocol_value_false = 1
protocol_value_true = 2
protocol_value_positive_int = 3
protocol_value_negative_int = 4
protocol_value_float = 5
protocol_value_string = 6
protocol_value_list = 7
protocol_value_map = 8
...
protocol_type_auth_challenge = 4
protocol_type_auth_response = 5
protocol_type_sealed = 6
...
local function create_socket_transport()
    ...
    local cdefOk, cdefErr = pcall(ffi.cdef, [[
        ...
        int select(int nfds, fd_set1* readfds, fd_set1* writefds, fd_set1* exceptfds,
    ...
    transport.tcp = function()
        local rawSocket = nil
        local receiveBuffer = ''
        local connectTimeout = 1.0  -- seconds; overwritten by settimeout()
        local wrapper = {}

        wrapper.settimeout = function(_, seconds)
        ...
        wrapper.connect = function(_, address, targetPort)
            ...
            -- Set non-blocking BEFORE connecting. Previously this happened
            -- AFTER connect() returned, which meant connect() itself ran
            -- fully blocking with no timeout at all - if nothing was
            -- listening (e.g. the viewer app not running) and the OS
            -- didn't refuse the connection instantly, this could freeze
            -- the entire game thread for many seconds (prerender runs on
            -- the game's own render loop). That's almost certainly what
            -- was presenting as a crash/freeze during character load.
            ...
            if result == 0 then
                -- Connected immediately (can happen for loopback).
                return true
            end

            local err = ws2.WSAGetLastError()
            if err ~= 10035 then  -- not WSAEWOULDBLOCK -> a real, immediate failure
                ws2.closesocket(rawSocket)
                rawSocket = nil
                return nil, 'connect failed: ' .. tostring(err)
            end

            -- Connection in progress (the normal case for a non-blocking
            -- connect). Wait up to connectTimeout for the socket to become
            -- writable, which signals the connect attempt has completed
            -- (successfully or not).
            local writeSet = ffi.new('fd_set1')
            writeSet.fd_count = 1
            writeSet.fd_array[0] = rawSocket

            local tv = ffi.new('struct timeval')
            tv.tv_sec = math.floor(connectTimeout)
            tv.tv_usec = math.floor((connectTimeout - math.floor(connectTimeout)) * 1000000)

            local selectResult = ws2.select(0, nil, writeSet, nil, tv)
            if selectResult == 0 then
                -- Timed out waiting - give up cleanly rather than blocking
                -- further, and rather than leaving a half-open socket
                -- around.
                ws2.closesocket(rawSocket)
                rawSocket = nil
                return nil, 'connect timed out'
            elseif selectResult < 0 then
                local selErr = ws2.WSAGetLastError()
                ws2.closesocket(rawSocket)
                rawSocket = nil
                return nil, 'select failed while connecting: ' .. tostring(selErr)
            end

            -- Socket is writable - check whether that means success or a
            -- delayed connection error (e.g. connection refused).
            local optval = ffi.new('int[1]', 0)
            local optlen = ffi.new('int[1]', ffi.sizeof('int'))
            ws2.getsockopt(rawSocket, SOL_SOCKET, SO_ERROR, ffi.cast('char*', optval), optlen)
            if optval[0] ~= 0 then
                ws2.closesocket(rawSocket)
                rawSocket = nil
                return nil, 'connect failed: ' .. tostring(optval[0])
            end
...
local npc_debug_enabled = false
local udp_send_debug_count = 0  -- caps the new UDP send diagnostic to 5 prints
local status_client = nil
local last_send = 0
local send_interval = 0.2
local player_valid_since = nil  -- os.clock() timestamp; nil until first seen valid
local status_warmup_seconds = 3.0  -- grace period after login before touching
                                     -- entity/party memory (get_mob_by_target,
                                     -- get_party, nearby npc scanning) - these
                                     -- read live game memory directly, and
                                     -- doing so during the character-select-
                                     -- to-world transition (while that memory
                                     -- may still be mid-update) is suspected
                                     -- to be the actual cause of a game
                                     -- freeze/crash on load, not the network
                                     -- code itself
local full_status_interval = 1.0
local last_full_status = 0
max_udp_payload_size = 60000
...
-- Port 8080 was found to be specifically intercepted on at least one
-- real setup (something on the system silently dropped UDP traffic to
-- it, even with general loopback UDP and other ports working fine) -
-- 8099 is now the default for new installs, but anyone with an old
-- settings.xml still has 8080 saved explicitly, which would otherwise
-- keep silently overriding the new default forever. Treat that specific
-- stale value as needing an upgrade, the same way as if no port had
-- been saved at all.
if port == 8080 then
    port = default_port
    if config then
...
-- Finishes setting up a socket that has just successfully connected,
-- whether that happened immediately or after being polled across
-- several frames. Factored out so both paths in connect_client() below
-- share the exact same post-connect logic.
...
local pending_connect_socket = nil
local pending_connect_start = 0
...
local function connect_client()
    ...
    -- Poll an already-in-progress non-blocking connect. A 0 timeout on
    -- select() returns immediately either way (that's what makes this
    -- safe to call every frame) - this replaces an earlier version that
    -- called select() with a 1 SECOND timeout instead. That sounds like
    -- a reasonable bound, but it isn't safe to block for even a bounded
    -- amount of time inside a per-frame render callback: it was stealing
    -- a full second of wall-clock time on EVERY frame for as long as
    -- nothing was listening, which is effectively ~1 FPS - completely
    -- indistinguishable from a freeze, and confirmed by the CONNTIMING
    -- diagnostic output showing exactly this pattern repeating forever.
    -- Spreading the same 1-second budget across many non-blocking polls
    -- (one per frame, each effectively instant) fixes this properly.
    if pending_connect_socket then
        local s = pending_connect_socket
        local elapsed = os.clock() - pending_connect_start
        local _, writable = socket.select(nil, { s }, 0)
        if writable and #writable > 0 then
            pending_connect_socket = nil
            print(('MogWatch RETRYDEBUG: connect succeeded at t=%.3f'):format(os.clock()))
            return finish_connect(s)
        elseif elapsed >= 1.0 then
            print(('MogWatch RETRYDEBUG: pending attempt timed out at t=%.3f'):format(os.clock()))
            report_connect_diag('command channel: connect timed out waiting for a response.')
            pcall(function() s:close() end)
            pending_connect_socket = nil
        end
        return false
    ...
    if not socket.select then
        report_connect_diag('command channel: this transport has no select() to poll a '
            .. 'non-blocking connect with - treating as unsupported rather than risking a hang.')
        return false
    end
    ...
    print(('MogWatch RETRYDEBUG: starting a fresh connect attempt at t=%.3f'):format(now))
    ...
    -- Genuinely in progress - store it and poll on subsequent frames
    -- above, rather than waiting inside this one.
    pending_connect_socket = s
    pending_connect_start = now
    return false
end
...
VANADIEL_EPOCH_UNIX = 1009810800
MOD32 = 4294967296 -- 2^32
...
npc_list_max_range = 50
npc_list_max_count = 15

local function build_nearby_npcs(player_x, player_z, player_y)
    if not player_x or not player_z then
        if npc_debug_enabled then
            print('MogWatch NPCDEBUG: player_x/player_z not available -- returning empty list.')
        end
        return {}
    end
    ...
    if not mobs then
        if npc_debug_enabled then
            print('MogWatch NPCDEBUG: get_mob_array() returned nil -- returning empty list.')
        end
        return {}
    end
    ...
    -- Exclude self and party members -- get_mob_array() returns every
    -- nearby entity, players included, not just monsters/NPCs. Cross-
    -- referenced by name against the player's own name and the current
    -- party roster.
    local exclude_names = {}
    local ok_player, self_player = pcall(windower.ffxi.get_player)
    if ok_player and self_player and self_player.name then
        exclude_names[self_player.name] = true
    end
    local ok_party, party = pcall(windower.ffxi.get_party)
    if ok_party and party then
        for _, member in pairs(party) do
            if type(member) == 'table' and member.name then
                exclude_names[member.name] = true
            end
        end
    end

    local passed_basic_filter = 0
    local excluded_by_name = 0
    local passed_range = 0

    local candidates = {}
    for _, mob in pairs(mobs) do
        if mob.valid_target and mob.name and mob.name ~= '' and mob.x and mob.z then
            if exclude_names[mob.name] then
                excluded_by_name = excluded_by_name + 1
            else
                passed_basic_filter = passed_basic_filter + 1
                -- mob.distance is the SQUARED distance, not the actual
                -- distance -- confirmed directly from Windower's own
                -- official "Distance" addon source (_addon.author =
                -- 'Windower'), which explicitly does
                -- "t.distance:sqrt()" before using it. Explains both
                -- symptoms exactly: the earlier "all NPCs disappeared"
                -- (squared values compared directly against the 50-yalm
                -- range threshold, so only would-be-negative-yalm mobs
                -- could ever pass) and "NPCs only show up within ~7
                -- yalms" (sqrt(50) = 7.07 -- the exact cutoff observed).
                local distance = mob.distance
                if type(distance) == 'number' then
                    distance = math.sqrt(distance)
                else
                    -- Fallback in case this field is ever genuinely
                    -- absent in some context -- a real 3D calculation
                    -- (including elevation), not the original 2D-only
                    -- version of this code.
                    local dx = mob.x - player_x
                    local dz = mob.z - player_z
                    local dy = (mob.y and player_y) and (mob.y - player_y) or 0
                    distance = math.sqrt(dx * dx + dy * dy + dz * dz)
                end
                if distance <= npc_list_max_range then
                    passed_range = passed_range + 1
    ...
    if npc_debug_enabled then
        print(('MogWatch NPCDEBUG: player=(%s,%s,%s) total_in_array=%d '
            .. 'passed_basic_filter=%d excluded_by_name=%d passed_range=%d '
            .. 'npc_list_max_range=%s'):format(
            tostring(player_x), tostring(player_y), tostring(player_z),
            total_mobs, passed_basic_filter, excluded_by_name, passed_range,
            tostring(npc_list_max_range)))
    end
...
local function build_status()
    ...
    if not player_valid_since then
        player_valid_since = os.clock()
    end
    local warmed_up = (os.clock() - player_valid_since) >= status_warmup_seconds
    ...
    if warmed_up then
        -- Only touch entity memory (get_mob_by_target, buffs) once the
        -- warm-up delay has passed - see status_warmup_seconds above.
        ...
        activeBuffs = build_active_buffs(player)
    end
    ...
    local status = {
        ...
        target = warmed_up and build_target(playerX, playerZ) or nil,
        npcs = warmed_up and build_nearby_npcs(playerX, playerZ, elevation) or nil,
    ...
    if warmed_up then
        -- get_party() also reads live entity memory for each member -
        -- same reasoning as get_mob_by_target above.
...
local function send_status()
    ...
    -- Byte-identical frames carry no new state, so normally skip the send
    -- to avoid spamming a quiet zone - but force one through periodically
    -- regardless, so a viewer app that's launched AFTER player state has
    -- already stabilized still gets data within full_status_interval
    -- seconds, instead of waiting indefinitely for something to change.
    local stale = (now - last_full_status) >= full_status_interval
    if encoded == last_sent_light_payload and not stale then
        return
    end
    ...
    if secure then
        ...
        local ok, sendErr = status_client:send(encoded)
        if udp_send_debug_count < 5 then
            udp_send_debug_count = udp_send_debug_count + 1
            print(('MogWatch UDP DEBUG: send() to %s:%d returned %s (bytes=%s, err=%s)')
                :format(host, port, tostring(ok), tostring(ok), tostring(sendErr)))
        end
        if not ok then
            close_status_client()
            return
...
local function normalize_item_name(item_name)
    -- Capitalize first letter of each word, but leave the rest of the
    -- word exactly as-is. Previously this force-lowercased everything
    -- after the first letter, which mangled roman numerals and acronyms
    -- (e.g. "Moglophone II" was coming out as "Moglophone Ii").
...
local function get_display_name(full_name)
    ...
    -- Fall back to the resource library's own casing when we can resolve
    -- an id, rather than returning our own reconstructed casing as-is -
    -- this is what fixes roman numerals/acronyms ("Moglophone II" instead
    -- of "Moglophone ii").
    local id = item_id_by_name[full_name] or item_id_by_name[normalized]
    if id and res.items[id] then
        return res.items[id].name_log or res.items[id].name or full_name
    end
...
function remove_item(item_name)
    ...
    if command_debug_enabled then
        print('MogWatch CMDDEBUG remove_item: received="' .. original_arg
            .. '" normalized="' .. item_name .. '" full_name="' .. full_name .. '"')
        print('MogWatch CMDDEBUG remove_item: in tracked_items=' .. tostring(tracked_items[full_name] ~= nil)
            .. ' in usable_items=' .. tostring(usable_items[full_name] ~= nil)
            .. ' in ammo_items=' .. tostring(ammo_items[full_name] ~= nil)
            .. ' in personal_items=' .. tostring(personal_items[full_name] ~= nil))
    end
...
local function check_for_drops(message, mode)
        ...
        if item_name then
            ...
            if item_name ~= "" and item_name ~= player_name then
                ...
                if auto_add_usable and is_usable_item(item_name) then
                    ...
                    announce('Counter: Stole usable item - ' .. get_display_name(item_name) .. '!')
                else
                    ...
                    track_increment(item_name, 1)
                    announce('Counter: Steal - ' .. get_display_name(item_name) .. '! Total: ' .. personal_counts[item_name])
                end
    ...
    if obtain_count and obtain_item and auto_add_personal then
        ...
        if count then
            ...
            -- Trigger the LootPopup overlay for personal drops too (chests,
            -- quest rewards, etc.), same as the enemy-drop trigger above.
            -- Use the resource library's own name when we can resolve an
            -- id, rather than our reconstructed casing, since normalize_
            -- item_name can't correctly handle things like roman numerals
            -- ("II" was coming out as "Ii").
            build_name_mappings()
            local popup_item_id = item_id_by_name[item_name] or 0
            local popup_item_name = item_name
            if popup_item_id ~= 0 and res.items[popup_item_id] then
                popup_item_name = res.items[popup_item_id].name_log or res.items[popup_item_id].name or item_name
            end
            windower.send_command('lootpopup show ' .. popup_item_id .. ' ' .. popup_item_name)
            ...
            if auto_add_usable and is_usable_item(item_name) then
                ...
                announce('Counter: Obtained ' .. count .. ' usable item - ' .. get_display_name(item_name) .. '!')
            else
                -- Add to personal drops
                if not personal_items[item_name] then
                    personal_items[item_name] = true
                ...
                personal_drop_times[item_name] = os.time()
                track_increment(item_name, count)
                announce('Counter: Obtained ' .. count .. ' ' .. get_display_name(item_name) .. '! Total: ' .. personal_counts[item_name])
            end
    ...
    if auto_add_personal then
        ...
        if obtained_item then
            ...
            -- Trigger the LootPopup overlay for this drop format too.
            build_name_mappings()
            local popup_item_id = item_id_by_name[item_name] or 0
            local popup_item_name = item_name
            if popup_item_id ~= 0 and res.items[popup_item_id] then
            ...
            if item_name:find("^Key Item:") then
                if not key_items[item_name] then
                    ...
                    announce('Counter: Key item obtained - ' .. get_display_name(item_name):gsub("^Key Item:%s*", "") .. '!')
                end
            -- Check if it's a usable item
            elseif auto_add_usable and is_usable_item(item_name) then
                if not usable_items[item_name] then
                    ...
                    usable_drop_times[item_name] = os.time()
                    track_increment(item_name, 1)
                    announce('Counter: Usable item obtained - ' .. get_display_name(item_name) .. '!')
                end
            else
                ...
                personal_drop_times[item_name] = os.time()
                track_increment(item_name, 1)
                announce('Counter: Personal drop - ' .. get_display_name(item_name) .. '! Total: ' .. personal_counts[item_name])
            end
    ...
    -- Mode 121: a separate message channel that at least some monsters
    -- use for what are semantically PERSONAL drops (same "<Player>
    -- obtains <item>" text shape as mode 127's enemy loot, but belongs
    -- in personal_items/personal_counts - the Personal tab - not
    -- tracked_items/the Drops tab). Found via //counter debugall on a
    -- drop mode 127 was silently missing.
    if mode == 121 and auto_add_personal then
        ...
        if player and item_match and (not player_name or player == player_name) then
            -- Temporary items show as "obtains the temporary item: X!"
            -- rather than the normal "obtains a X." - strip that
            -- descriptive prefix so we get the real item name.
            item_match = item_match:gsub("^[Tt]he [Tt]emporary [Ii]tem:%s*", "")
            item_match = item_match:gsub("[%.!%?]+$", ""):gsub("^%s+", ""):gsub("%s+$", "")
            local item_name = normalize_item_name(item_match)

            if debug_mode then
                windower.add_to_chat(COUNTER_COLOR, string.format('Counter DEBUG: Mode 121 personal drop - Player: "%s", Item: "%s" -> "%s"', player, item_match, item_name))
            end
    ...
    if mode == 127 then
        if debug_mode then
            windower.add_to_chat(COUNTER_COLOR, string.format('Counter DEBUG: Mode %d detected!', mode))
        ...
        if clean_message:lower():find("obtain") then
            ...
            if player and item_match then
                ...
                -- Trigger the LootPopup overlay addon for EVERY self-drop
                -- caught by this pattern, regardless of whether Counter is
                -- tracking/auto-adding it. LootPopup is a fully separate
                -- addon (addons/lootpopup/) - this just "shouts" at it via
                -- windower.send_command, same as typing the command in the
                -- console. build_name_mappings is a no-op after the first
                -- call. item_id_by_name comes from MogWatch's own existing
                -- resource cache; if a name isn't found (rare - some
                -- resource name mismatch) we still fire with id 0 so
                -- LootPopup falls back to text-only instead of not showing
                -- anything.
                build_name_mappings()
                local popup_item_id = item_id_by_name[normalized_item] or 0
                local popup_item_name = normalized_item
                if popup_item_id ~= 0 and res.items[popup_item_id] then
                    popup_item_name = res.items[popup_item_id].name_log or res.items[popup_item_id].name or normalized_item
                end
                windower.send_command('lootpopup show ' .. popup_item_id .. ' ' .. popup_item_name)
                ...
                if auto_add_usable and is_usable_item(normalized_item) and not usable_items[normalized_item] then
                    ...
                    announce('Counter: Auto-added "' .. get_display_name(normalized_item) .. '" to usable items.')
                    save_settings()
                    update_display()
                    return
                end
                ...
                if auto_add_drop and not tracked_items[normalized_item] and not usable_items[normalized_item] and not ammo_items[normalized_item] then
                    ...
                    if is_ammo_item(normalized_item) then
                        ...
                        announce('Counter: Auto-added ammo "' .. get_display_name(normalized_item) .. '" to item drops.')
                    else
                        tracked_items[normalized_item] = true
                        item_counts[normalized_item] = 0
                        announce('Counter: Auto-added "' .. get_display_name(normalized_item) .. '" to tracking list.')
                    end
                end
                ...
                if tracked_items[normalized_item] then
                    ...
                    track_increment(normalized_item, 1)
                    announce('Counter: ' .. get_display_name(normalized_item) .. ' dropped! Total: ' .. item_counts[normalized_item])
                    save_settings()
                    update_display()
                elseif usable_items[normalized_item] then
                    usable_drop_times[normalized_item] = os.time()
                    track_increment(normalized_item, 1)
                    announce('Counter: Usable item ' .. get_display_name(normalized_item) .. ' dropped!')
                    save_settings()
                    update_display()
                elseif ammo_items[normalized_item] then
                    ammo_drop_times[normalized_item] = os.time()
                    track_increment(normalized_item, 1)
                    announce('Counter: Ammo ' .. get_display_name(normalized_item) .. ' dropped!')
                    save_settings()
                    update_display()
                else
                    if debug_mode then
...
windower.register_event('addon command', function(...)
    ...
    if sub == 'npcdebug' then
        npc_debug_enabled = not npc_debug_enabled
        print(('MogWatch addon: nearby NPC debug %s.'):format(
            npc_debug_enabled and 'ON' or 'OFF'))
        return
    end
...
-- Diagnostic flag from earlier isolation tests, confirmed the freeze is
-- fixed by status_warmup_seconds above (no freeze even without this flag).
-- Left here in case it's ever useful again, but should stay false.
local DISABLE_NETWORKING_FOR_TESTING = false

windower.register_event('prerender', function()
    if DISABLE_NETWORKING_FOR_TESTING then
        return
    end
    local now = os.clock()
    ...
    local rt1 = os.clock()
    if (rt1 - rt0) > 0.05 then
        print(('MogWatch FRAMETIMING: receive_commands took %.3fs'):format(rt1 - rt0))
    end
    ...
    if (now - last_send) >= send_interval then
        ...
        local st1 = os.clock()
        if (st1 - st0) > 0.05 then
            print(('MogWatch FRAMETIMING: send_status took %.3fs'):format(st1 - st0))
        end
        last_send = now
    end
end)
