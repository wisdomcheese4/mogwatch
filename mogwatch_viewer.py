...
NPC_TRACKING_FILE = os.path.join(app_dir(), 'mogwatch_npc_tracking.json')
...
class NpcWindow(tk.Toplevel):
    ...
    def __init__(self, master):
        ...
        self.tree.tag_configure('tracked', foreground=MP_COLOR)
        self.tree.pack(fill='both', expand=True, padx=10, pady=(0, 6))
        self.tree.bind('<Button-1>', self._on_row_click)

        lists_row = tk.Frame(self, bg=BG)
        lists_row.pack(fill='x', padx=10, pady=(0, 6))
        tk.Label(lists_row, text='List:', bg=BG, fg=MUTED,
                 font=('Segoe UI', 8)).pack(side='left')
        self.list_var = tk.StringVar(value='')
        self.list_combo = ttk.Combobox(lists_row, textvariable=self.list_var,
                                        state='readonly', width=14)
        self.list_combo.pack(side='left', padx=(4, 4))
        ttk.Button(lists_row, text='Load', command=self._load_list).pack(side='left')
        ttk.Button(lists_row, text='Save As...', command=self._save_list_as).pack(
            side='left', padx=(4, 0))
        ttk.Button(lists_row, text='Delete', command=self._delete_list).pack(
            side='left', padx=(4, 0))
    ...
    # -- Saved tracking lists (persisted to disk, shared across restarts) --

    @staticmethod
    def _load_all_lists():
        try:
            with open(NPC_TRACKING_FILE, 'r', encoding='utf-8') as f:
                data = json.load(f)
    ...
    @staticmethod
    def _save_all_lists(all_lists):
        try:
            with open(NPC_TRACKING_FILE, 'w', encoding='utf-8') as f:
                json.dump(all_lists, f, indent=2)
        except OSError:
            pass

    def _refresh_list_combo(self):
        all_lists = self._load_all_lists()
        self.list_combo.config(values=sorted(all_lists.keys()))

    def _load_list(self):
        name = self.list_var.get().strip()
        if not name:
            return
        all_lists = self._load_all_lists()
        names = all_lists.get(name)
        if isinstance(names, list):
            self.tracked_names = set(names)
            self._render()

    def _save_list_as(self):
        name = simpledialog.askstring('Save Tracking List', 'Name for this list:', parent=self)
        if not name:
            return
        name = name.strip()
        if not name:
            return
        all_lists = self._load_all_lists()
        all_lists[name] = sorted(self.tracked_names)
        self._save_all_lists(all_lists)
        self._refresh_list_combo()
    ...
    # Windower sometimes reports an entity's name as this generic
    # placeholder before the real name has resolved (e.g. right as it
    # spawns or comes back into range). Since tracking matches by name
    # (necessary for saved lists to survive a zone change or relog, where
    # internal IDs don't persist), tracking this literal placeholder would
    # match every other entity currently showing the same placeholder too,
    # not just the one intended.
    GENERIC_NAMES = {'npc'}

    def _confirm_generic_name(self, name):
        if name.strip().lower() not in self.GENERIC_NAMES:
            return True
        return messagebox.askyesno(
            'Generic Name',
            '"%s" is a placeholder name Windower shows before an entity\'s '
            'real name has loaded -- tracking it will match every other '
            'entity currently showing that same placeholder, not just the '
            'one you meant. Track it anyway?' % name)

    def _track_from_entry(self):
        name = self.track_entry.get().strip()
        if not name:
            return
        if not self._confirm_generic_name(name):
            return
    ...
    def _render(self):
        ...
        visible_npcs = [
            npc for npc in self._last_npcs
            if (npc.get('name') or '').strip().lower() not in self.GENERIC_NAMES
        ]
        ...
        sorted_npcs = sorted(visible_npcs, key=sort_key)
        ...
        for npc in sorted_npcs:
            ...
            tags = ('tracked',) if name in self.tracked_names else ()
...
def main():
    ...
    parser.add_argument('--port', type=int, default=8099,
                         help='UDP port to listen on (must match //mogwatch '
                              'port on the addon side). Default 8099.')
