...
class NpcWindow(tk.Toplevel):
    """Plain list of nearby NPCs/mobs -- name, distance, HP% -- no map
    involved. Filtered and capped by the addon (within 50 units, top 15
    closest) to keep frames small."""
    ...
    def __init__(self, master):
        ...
        for col, width, heading in zip(columns, (180, 90, 80), ('Name', 'Distance', 'HP')):
    ...
    def update_payload(self, payload):
        ...
        for npc in payload.get('npcs') or []:
            ...
            self.tree.insert('', 'end', values=(name, dist_text, hp_text),
...
class PartyWindow(tk.Toplevel):
    ...
    BASE_HEIGHT = 60           # title label + padding, before any member rows
...
class ViewerApp:
    ...
    def _set_window_icon(self, root):
        # mogwatch.ico needs to sit next to the script (or, once packaged,
        # next to the .exe) -- app_dir() already resolves correctly for
        # both cases. iconbitmap with a .ico is Windows-specific; harmless
        # no-op (caught) on any other platform or if the file's missing.
        icon_path = os.path.join(app_dir(), 'mogwatch.ico')
        try:
